...
function Task(props) {
  ...
  const complete = () => {
    ...
    axios
      .post("hhttps://task-tracker-backend-production.up.railway.app/complete", upobj)
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
  ...
  const handleDelete= ()=>{
    const upobj = {
      ...
      axios.post("https://task-tracker-backend-production.up.railway.app/delete", upobj)
      .then(res=>{
          console.log(res)
      })
      .catch(err=>{
  ...
  return (
    <div className="task">
      ...
      <h4 className="task_desc" style={taskStyle}>Desc : {task.desc} </h4>
